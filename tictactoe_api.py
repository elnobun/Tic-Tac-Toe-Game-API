# -*- coding: utf-8 -*-`
"""tictactoe_api.py - Create and configure the Game API exposing the resources.
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    def new_game(self, request):
        ...
        if not (player_x and player_o):
            ...
            raise endpoints.NotFoundException(
                    'User % does not exist!' % wrong_user.name)
        # Board Size is 3. Check if board size is valid.
        boardSize = 3
    ...
    def cancel_game(self, request):
        """Cancel a game while game is in progess"""
    ...
    def _update_finished_games():
        ...
        if games:
            count = len(games)
            memcache.set(MEMCACHE_GAMES_PLAYED, 'The games finished number is %s' % count)
